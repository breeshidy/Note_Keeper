...
public class NextThroughNotesTest {
    ...
    public void NextThroughNotes(){
        ...
        //ensure notes are selected
        onView(withId(R.id.nav_view)).perform(NavigationViewActions.navigateTo(R.id.nav_note));

        //select the first note from our recycler view
        onView(withId(R.id.list_items)).perform(RecyclerViewActions.actionOnItemAtPosition(0,click()));

        //verifying the correct note is open. checking that it has the correct values
        List<NoteInfo> notes = DataManager.getInstance().getNotes();
         for (int index = 0; index<notes.size(); index++){
            ...
            //ensure each note is clicked when action_next is enabled
